...
# ---------------------------------------------------------------------------
#
# Scoped to THIS cluster's name rather than a blanket autoscaling:* grant — the policy
# the module generates keys its write actions (SetDesiredCapacity, TerminateInstanceIn
# AutoScalingGroup) to ASGs tagged for this cluster, so the role cannot resize an
# unrelated node group in the same account.
