...
> **Live deployment log (account 064271146369, started 2026-09-22):**
> - [x] Root MFA enabled (user's own action, confirmed via `aws iam get-account-summary`)
> - [x] Task 0 Step 1 (AWS Budgets guardrail): created `helpdesk-eks-guardrail`, $180/mo, 50/80/100% email alerts to adnan.akram@mindstormstudios.com
> - [ ] Task 0 Step 2 (4 remaining $20 credit tasks — EC2 launch/terminate, RDS config, Lambda deploy, Bedrock prompt) — not done yet
> - [ ] **Task 1 (dedicated `terraform-eks-admin` IAM user) — BLOCKED.** The keys handed to this session were the account's **root** access keys, not an IAM user's. Root has no MFA-independent permission boundary and shouldn't run Terraform day-to-day. Creating the IAM user was attempted twice and denied both times by the harness's own permission classifier (IAM/credential-mutation commands need an explicit settings-level allow, not just a chat go-ahead) — needs either a Bash permission rule added on the user's machine, or the user creating `terraform-eks-admin` (attach `AdministratorAccess`, generate an access key) via the AWS Console directly. Until resolved, `terraform apply` has **not** been run — only `terraform plan` (read-only) has, still under root.
> - [x] Found and fixed a real bug via the live account before it could cost anything: `kubernetes_version` was pinned to `1.31`, which (checked via `aws eks describe-cluster-versions`) rolled into `EXTENDED_SUPPORT` on 2025-11-26 — would have billed the control plane at $0.60/hr instead of $0.10/hr, the exact trap this doc's Global Constraints already warned about in the abstract. Bumped to `1.36`.
> - [x] `terraform init` + `terraform plan` against the real account: clean, **86 resources to add, 0 to change, 0 to destroy**, no permission errors, no NAT Gateway or ECR resources present (confirms the cost-conscious defaults are actually taking effect). Plan saved to `infra/terraform/tfplan` (gitignored, not portable across machines/state — regenerate with `terraform plan -out=tfplan` if picking this up elsewhere).
> - [ ] `terraform apply` — **not run yet**, waiting on the IAM user blocker above so it runs as a scoped user, not root.
